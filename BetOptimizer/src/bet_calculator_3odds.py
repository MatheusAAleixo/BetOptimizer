...
def calcular_apostas(valor_total, odd_aposta1, odd_aposta2, odd_aposta3):
    ...
    for centavos_aposta1 in range(1, int(valor_total * 100)):
        ...
        for centavos_aposta2 in range(1, int((valor_total - aposta1) * 100)):
            aposta2 = Decimal(centavos_aposta2) / Decimal(100)
    ...
    if melhor_retorno > valor_total:
        return melhor_aposta1, melhor_aposta2, melhor_aposta3, melhor_retorno
    else:
        return None
...
def calcular_e_mostrar_resultado(event=None):
    try:
        ...
        odd_aposta3 = Decimal(odd_aposta3_entry.get().replace(',', '.'))

        resultado = calcular_apostas(valor_total, odd_aposta1, odd_aposta2, odd_aposta3)

        if resultado:
            aposta1, aposta2, aposta3, melhor_retorno = resultado
            resultado_label.config(text=f"Aposta 1: Apostei {aposta1:.2f}, Retorno: {melhor_retorno:.2f}\n"
                                        ...
                                        f"Aposta 3: Apostei {aposta3:.2f}, Retorno: {melhor_retorno:.2f}")
        else:
...
odd_aposta3_label = ttk.Label(frame, text="Odd Aposta 3:")
odd_aposta3_entry = ttk.Entry(frame)
...
odd_aposta3_label.grid(row=3, column=0, padx=10, pady=10, sticky=tk.W)
odd_aposta3_entry.grid(row=3, column=1, padx=10, pady=10, sticky=tk.E)
calcular_button.grid(row=4, column=0, columnspan=2, pady=10)
resultado_label.grid(row=5, column=0, columnspan=2, pady=(10, 0))
